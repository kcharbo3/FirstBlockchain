...
class Blockchain:
    ...
    def createGenesisBlock(self):
        ...
        return Block([Transaction([cbOutput], [None], "14b23e638f3abca17e920c3c75eda82d3a84d9d3", 0, 0, self.coinbaseReward)], "0", 0)
    ...
    def addBlock(self, newBlock):
        ...
        if not self.acceptLargestChain():
    ...
    def validateBlock(self, block):
        ...
        if int(block.getHeader(), 16) >= self.getTargetNum():
            return False
        if block.getDifficulty() != self.getTarget():
            return False
    ...
    def registerNode(self, nodeAddress):
        ...
        self.nodes = self.nodes + parsed_url.netloc
...
class Output:
    ...
    def __init__(self, amount, lockingScript, tHash, ind):
        ...
        self.txHash = tHash
        self.index = ind
        hashable = self.txHash + str(self.index)
        if len(hashable) % 2 == 1:
            hashable = hashable + '0'
        self.identifier = hashlib.sha256(binascii.unhexlify(hashable)).hexdigest()
...
class Transaction:
    ...
    def getToAddress(self):
        return self.crypto.getAddressPubKH(self.to_pubKeyH)

    def getFromAddress(self):
        if len(self.oldOutputs) == 0:
            return 'Coinbase tx'
        out = self.oldOutputs[0]
        return self.crypto.getAddressPubKH(out.getPKHash())
