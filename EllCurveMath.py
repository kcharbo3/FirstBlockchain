...
#should be an interface not object
class EllCurveMath:
    ...
    def getPubKeyHash(self, pubK):
        ...
        ripeMD.update((hashlib.sha256((pubK).decode('hex')).hexdigest()).decode('hex'))
        hashedK = ripeMD.hexdigest()
        return hashedK
    
    def getAddress(self, pubK):
        prefixHashed = "00" + self.getPubKeyHash(pubK)
        checksum = hashlib.sha256((hashlib.sha256(prefixHashed.decode('hex')).hexdigest()).decode('hex')).hexdigest()
        checksum = checksum[:8]
